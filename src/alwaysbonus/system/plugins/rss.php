...
while($row = mysql_fetch_array($result))
{
        if(!empty($row["coupon_image"]))
	        ...
	        $image_url = DOCROOT.'themes/'.CURRENT_THEME.'/images/no image.jpg';
        
	$xml_content .= "	
	        <item>
                        <title>".html_entity_decode(strip_tags($row['coupon_name']))."</title>
						<link><![CDATA[".DOCROOT."setcity.php?cityid=".$row['cityid']."&cityname=".html_entity_decode(strip_tags($row['cityname']))."]]></link>
                        <description><![CDATA[ <p>".html_entity_decode(strip_tags($row['coupon_description']))."</p>"
							...
							."<p>Buy now: $".html_entity_decode(strip_tags($row['coupon_realvalue']/2))."</p>]]>
						</description>
	...
	{		      
        ...
        $xml_content .= "
                        ...
                        <url>".DOCROOT."deals/".friendlyURL(html_entity_decode($row["coupon_name"]))."_".$row["coupon_id"].".html</url>
